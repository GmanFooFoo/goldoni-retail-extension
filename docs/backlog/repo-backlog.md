# Repo-Backlog — Goldoni Retail Extension

> Status-Tabelle im MASCHIN-Format. `#` = Referenz-ID (nicht Priorität).
> Prio: P1 / P2 / P3 / Backlog. Effort: XS/S/M/L/XL.
> Status: Offen / In Progress / Blocked / Done.

## P1 — Diese Session (2026-04-10)

| # | Item | Prio | Effort | Wer | Blocker? | Status | Impact |
|---|---|---|---|---|---|---|---|
| 1 | Repo-Setup (Stufe 1a): .gitignore, CLAUDE.md, README, session-state, backlog, rollout-plan, initial commit | P1 | S | German | — | ✅ Done | Arbeitsgrundlage für alle weiteren Stufen |
| 2 | GitHub-Repo `GmanFooFoo/goldoni-retail-extension` privat anlegen + push | P1 | XS | German | — | ✅ Done | Remote-Backup |
| 3 | Labels anlegen: `feedback-silvio`, `quality-issue`, `new-info`, `decision-needed` | P1 | XS | German | — | ✅ Done | Issue-Taxonomie |
| 4 | ~~MkDocs Material Reader-Site (Stufe 1b)~~ | — | — | — | — | ❌ Dropped | MkDocs war Overkill; User will plain markdown |
| 5 | ~~Vercel-Deploy `retail.restaurante-goldoni.de` (Stufe 1c)~~ | — | — | — | — | ❌ Dropped | Kein Hosting. Nur GitHub + lokal |
| 6 | Persona-Refresh: 9 Personas (mit Namen) + README-Rewrite + rollout-plan-Rewrite für Silvio + Ton-Reset (Marcello-Persona gestrichen) | P1 | L | German | — | ✅ Done | Silvio-facing Dokumente im Freundschafts-Ton, Personas bereit für Stufe 3 |
| 6b | Frontmatter-Cleanup aus allen MD-Dateien (Obsidian-Leftover) | P1 | XS | German | #6 | ✅ Done | 19 Business-Case-Docs ohne Frontmatter |
| 6c | "Goldoni"-Präfix aus 19 Original-Dateinamen entfernen (H1 bleibt original) | P1 | S | German | #6b | ✅ Done | Lesbarere Navigation |
| 6d | Glossar-Admonitions auf GitHub-Blockquotes umstellen | P1 | XS | German | — | ✅ Done | Glossar rendert sauber auf GitHub |
| 7 | **Stufe 3 — Deep Review der Business-Case-Dokumente** | P1 | XL | German | #6 | 🔲 Offen (neue Session) | Empfohlener Einstieg: Pilot an Doc 02. Siehe SESSION-PROMPT-NEXT.md |
| 12 | Session-Report + Next-Session-Prompt (Stufe 4) | P1 | S | German | #6 | ✅ Done | Handoff für Stufe 3 liegt in SESSION-PROMPT-NEXT.md |

## P2 — Nächste Session

| # | Item | Prio | Effort | Wer | Blocker? | Status | Impact |
|---|---|---|---|---|---|---|---|
| 20 | ~~v2-Rewrite der 5 kritischen Docs auf Basis der Revisionspläne~~ | — | — | — | — | ✅ Done | Session 13 + 14: 19 von 20 Docs auf v2 |
| 21 | ~~Review der 🟡 lückenhaften Docs (01, 07, 09, 11, 14, 15, 16, 18)~~ | — | — | — | — | ✅ Done | Session 13: alle 20 Docs lead-reviewt, Session 14: alle v2 geschrieben |
| 22 | ~~Doc 06 LMIV-Text-v2~~ **Aufgelöst (Session 15, 2026-04-13):** Doc 06 v2 mit 13-Felder-LMIV-Compliance-Checkliste geschrieben, echter Name Silvio Brunetti integriert. Visueller Entwurf bleibt auf SP-09/10/11/19 + Vetamt-Reg-Nr. geblockt — dokumentiert als Abhängigkeits-Tabelle in Doc 06 selbst. | — | — | — | — | ✅ Done | Doc 06 v2 vollständig, visueller Mockup bewusst auf Silvio-Input vertagt |

## P3 — Silvio-Fragen (werden als GitHub Issues angelegt)

| # | Item | Prio | Effort | Wer | Blocker? | Status | Impact |
|---|---|---|---|---|---|---|---|
| 30 | Silvios echter Nachname, Adresse, Steuernummer | P3 | XS | Silvio | — | Offen | Ersetzt `[TBD-Silvio]` überall |
| 31 | MwSt-Vorklärung mit Steuerberater: 7% oder 19%? | P3 | S | Silvio + Frau Keller | — | Offen | Kippt Wirtschaftlichkeit Doc 02 |
| 32 | Vorgespräch mit Veterinäramt Stuttgart: welcher Antragspfad? | P3 | S | Silvio | — | Offen | Timing Rollout Doc 13 |
| 33 | Metro-Angebote für Vakuumiergerät + Schockfroster (Lieferzeit?) | P3 | S | Silvio | — | Offen | Timing Rollout Doc 13 |
| 34 | ~~Domain-DNS bei DomainFactory: CNAME für `retail.restaurante-goldoni.de`~~ | — | — | — | — | ❌ Dropped | Hosting entfällt |

## Backlog

| # | Item | Prio | Effort | Wer | Blocker? | Status | Impact |
|---|---|---|---|---|---|---|---|
| 50 | Scoring-Matrix Claudia (Stammgast) als wiederverwendbares Tool | Backlog | S | German | — | Offen | Skalierbare Kundenbefragung |
| 51 | Brand-Konsistenz-Checkliste Jana als Review-Tool | Backlog | S | German | — | Offen | Wiederverwendbar für weitere Goldoni-Projekte |
| 52 | Sensitivity-Analyse Doc 02 (Break-Even bei 30/40/50/60 Einheiten/Woche) → [Analyse](../plans/52-sensitivity-analyse.md) | Backlog | S | German | #31 (MwSt offen, als Worst-Case mitgerechnet) | Done (2026-04-14) | Risiko-Transparenz, 30-Stk-Szenario als kritische Kipp-Linie, Go/No-Go-Checkpoints bis Q1 2027 |
| 53 | Rechtscheck Produkthaftung (Doc 14) durch echten Anwalt | Backlog | M | Silvio + Anwalt | — | Offen | Haftungsrisiko Silvio persönlich |
| 54 | Geschenkebox-Konzept "Sugo + Rummo-Nudeln" durchplanen: LMIV-Kennzeichnung, Haftungs-Profil, Kalkulation, Verpackungs-Design. → [Konzept](../plans/54-geschenkebox-konzept.md) | Backlog | M | German + Brand/Marketing | SP-20a/b (Silvio-OK + Rummo-Wahl) | Done (Konzept 2026-04-14, Cross-Ref-Updates nach Silvio-OK) | VK 19,90 € Einstieg / 22,90 € Premium, DB 40–49 % netto, Launch Monat 4. Auflöst Inconsistency #16. |
| 55 | BAFA-Berater-Registrierung für German abschließen: Seminar besucht (angemeldet 2026-04-12), danach Registrierung im BAFA-Portal über [leitstelle.org](https://www.leitstelle.org/wie-werde-ich-bafa-berater). Voraussetzung: >50 % Umsatz aus Beratung (passt), QM-Handbuch, Beratererklärung. Erst nach Registrierung kann BAFA-Antrag für Silvio gestellt werden (D-11). | Backlog | S | German | Seminar-Ergebnis | Offen | Ermöglicht 50 % Zuschuss auf Germans Beratungsleistung (max. 1.750 €). Programm läuft bis 31.12.2026. |
| 56 | Q4Me QM-Software evaluieren: Preismodell, Funktionsumfang (HACCP-Doku, Chargen-Protokolle, Temperatur-Logs, Reinigungspläne, Schulungsnachweise), Eignung für Einzel-Gastronom. Cross-Ref Doc 05 HACCP, Doc 22 Software-Tools. → [Evaluation](../plans/22-q4me-evaluation.md) | Backlog | S | German | — | Done (Eval 2026-04-14, Test-Phase folgt in Rollout-Woche 3–6) | Digitalisierung der Papier-Protokolle, 30 €/Monat Cashflow-Annahme in Doc 02 bestätigt, DEHOGA-Status → SP-25 |
